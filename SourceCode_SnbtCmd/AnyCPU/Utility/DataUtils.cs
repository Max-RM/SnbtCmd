...
namespace TryashtarUtils.Utility
{
    ...
    {
        private static readonly Dictionary<string, Tuple<double, float>> SpecialFloatingPoints = new Dictionary<string, Tuple<double, float>>(StringComparer.OrdinalIgnoreCase)
        {
            { "∞", Tuple.Create(double.PositiveInfinity, float.PositiveInfinity) },
            { "+∞", Tuple.Create(double.PositiveInfinity, float.PositiveInfinity) },
            { "-∞", Tuple.Create(double.NegativeInfinity, float.NegativeInfinity) },
            { "Infinity", Tuple.Create(double.PositiveInfinity, float.PositiveInfinity) },
            { "+Infinity", Tuple.Create(double.PositiveInfinity, float.PositiveInfinity) },
            { "-Infinity", Tuple.Create(double.NegativeInfinity, float.NegativeInfinity) },
            { "NaN", Tuple.Create(double.NaN, float.NaN) }
        };

        public static double? TryParseSpecialDouble(string value)
        {
            Tuple<double, float> result;
            if (SpecialFloatingPoints.TryGetValue(value, out result))
                return result.Item1;
            return null;
        }
